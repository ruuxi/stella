...
Split-repo AI assistant: `desktop/` (Electron + React/Vite) and `backend/` (Convex serverless). No `.git` at root — each sub-repo has its own git repository. Desktop owns the PI agent runtime, local tool catalog, local first data, and all agent execution. Backend owns the llm calls.
...
Storage is split between local desktop and backend Convex — see each sub-repo's AGENTS.md for details.
