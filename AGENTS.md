...
Split-repo AI assistant: `desktop/` (Electron + React/Vite) and `backend/` (Convex serverless). Desktop owns the PI agent runtime, local tool catalog, local first data, and all agent execution. Backend owns the llm calls and offline fallback orchestrator.
