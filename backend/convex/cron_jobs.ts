...
export const run = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.scheduler.runAfter(0, internal.cron_jobs.execute, {
      jobId: job._id,
      forced: true,
    });
    return job;
...
export const tick = internalAction({
  ...
  handler: async (ctx) => {
    ...
    for (const job of due) {
      ...
      await ctx.runMutation(internal.cron_jobs.markRunning, {
        id: job._id,
        runningAtMs: now,
      });
      await ctx.scheduler.runAfter(0, internal.cron_jobs.execute, {
...
export const execute = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    if (!args.forced && job.nextRunAtMs > now) {
      await ctx.runMutation(internal.cron_jobs.finishRun, {
        id: job._id,
        runningAtMs: undefined,
    ...
    if (!shouldDelete) {
      ...
      await ctx.runMutation(internal.cron_jobs.deleteJob, { jobId: job._id });
    }
