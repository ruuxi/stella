...
export const tick = internalAction({
  ...
  handler: async (ctx) => {
    ...
    for (const config of due) {
      ...
      const nextRunAtMs = now + intervalMs;
      await ctx.runMutation(internal.heartbeat.markScheduled, {
        id: config._id,
        nextRunAtMs,
        lastRunAtMs: now,
      });
      await ctx.scheduler.runAfter(0, internal.heartbeat.run, {
        configId: config._id,
        reason: "interval",
      });
    }
...
export const run = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    if (!isWithinActiveHours(config.activeHours, now)) {
      await ctx.runMutation(internal.heartbeat.recordRun, {
        id: config._id,
