...
type DeviceToolName = (typeof DEVICE_TOOL_NAMES)[number];
...
const executeDeviceTool = async (
  ...
  if (!resultEvent) {
    return `Tool ${toolName} timed out after ${Math.round(
...
export const createDeviceTools = (ctx: ActionCtx, context: DeviceToolContext) => {
  ...
  return {
    ...
    TaskOutput: tool({
      ...
      execute: (args) => call("TaskOutput", args),
    }),
