...
Convex Return Validators — When to Use
Default: Skip returns validators. Rely on TypeScript types and inference.

Why not "always":

Verbose inline validators get copy-pasted, making schema changes a multi-site fix
Token-expensive — LLMs re-read them constantly, wasting context window
LLMs hallucinate fields or pick wrong validator types when reproducing schemas
System fields (_id, _creationTime) get re-declared everywhere
Use returns when:

Exact runtime contract needed — preventing extra fields from leaking (TypeScript's structural typing won't catch this)
any or unvalidated external data flows through the return path
Component codegen — inference unavailable, validator is the contract
Static codegen — functions default to v.any() without explicit returns
OpenAPI generation — missing validators become any in the spec
If you do use them:

Extract shared validators into reusable constants instead of inlining
Use convex-helpers validator utilities to avoid re-declaring system fields
Use Doc<> and WithoutSystemFields type helpers where possible
