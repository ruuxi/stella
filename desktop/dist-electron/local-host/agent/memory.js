 ...
 */
export function createProxyEmbedder(proxyUrl, auth) {
    return async (text) => {
 ...
 */
export function recallMemories(ownerId, queryEmbedding) {
 ...
 */
export function saveMemory(ownerId, content, embedding, conversationId) {
    // Dedup check
    if (embedding) {
...
export function decayOldMemories() {
    ...
    const result = rawRun("DELETE FROM memories WHERE accessed_at < ?", [cutoff]);
