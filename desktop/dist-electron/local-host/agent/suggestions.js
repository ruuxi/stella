...
export async function generateSuggestions(conversationId, ownerId) {
    try {
        // Load command catalog
        const catalog = rawQuery("SELECT command_id, name, description FROM commands WHERE enabled = 1", []);
        if (catalog.length === 0)
            return;
        // Load recent messages
        const events = rawQuery("SELECT type, payload FROM events WHERE conversation_id = ? AND type IN ('user_message', 'assistant_message') ORDER BY timestamp DESC LIMIT 10", [conversationId]);
        const messageParts = [];
