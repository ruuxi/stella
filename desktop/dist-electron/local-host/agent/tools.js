...
import { createTask, cancelTask, getTaskById, } from "./tasks_local";
const log = (...args) => console.log("[local-tools]", ...args);
/**
 ...
 */
export function createLocalTools(options) {
    const { agentType, toolsAllowlist, maxTaskDepth, ownerId, conversationId, deviceId, toolHost, } = options;
    const tools = {};
    ...
    for (const toolName of deviceToolNames) {
        if (toolsAllowlist && !toolsAllowlist.includes(toolName))
            continue;
        tools[toolName] = tool({
            ...
            execute: async (args) => {
                ...
                // Execute in-process
                const result = await toolHost.executeTool(toolName, args, {
                    conversationId,
                    deviceId,
                    requestId,
                    agentType,
    ...
    if (!toolsAllowlist || toolsAllowlist.includes("OpenCanvas")) {
        tools["OpenCanvas"] = tool({
            ...
            execute: async ({ name, title, url }) => {
                ...
                // Update canvas state
                const now = Date.now();
                const existing = rawQuery("SELECT id FROM canvas_states WHERE owner_id = ? AND conversation_id = ?", [ownerId, conversationId]);
                if (existing.length > 0) {
                    update("canvas_states", { name, title: title || null, url: url || null, updated_at: now }, { id: existing[0].id });
                }
                else {
    ...
    if (!toolsAllowlist || toolsAllowlist.includes("RecallMemories")) {
        tools["RecallMemories"] = tool({
            ...
            execute: async ({ query }) => {
                ...
                const memories = rawQuery("SELECT content, accessed_at FROM memories WHERE owner_id = ? ORDER BY accessed_at DESC LIMIT 10", [ownerId]);
    ...
    if (!toolsAllowlist || toolsAllowlist.includes("ActivateSkill")) {
        tools["ActivateSkill"] = tool({
            ...
            execute: async ({ skillId }) => {
                const rows = rawQuery("SELECT markdown, name FROM skills WHERE skill_id = ? AND enabled = 1 LIMIT 1", [skillId]);
                if (rows.length === 0)
                    return `Skill "${skillId}" not found or not enabled`;
