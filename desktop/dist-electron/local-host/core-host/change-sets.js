...
import { defaultValidationSpecs, runValidations, summarizeValidationResults, } from "./validations.js";
const MAX_DIFF_PATCH = 300000;
const truncatePatch = (value) => {
...
const getSnapshotForDiffEntry = (entry) => entry.after ?? entry.before;
export const createChangeSetManager = (options) => {
    ...
    const ensureBaseline = async () => {
        ...
        if (existing) {
            const snapshot = await stateStore.loadBaselineSnapshot(existing.baselineId);
            if (snapshot) {
    ...
    const loadActiveChangeSetRecord = async () => {
        ...
        if (!active)
            return null;
        const record = await stateStore.loadChangeSetRecord(active.id);
        return record;
    };
    const saveRecord = async (record) => {
        await stateStore.saveChangeSetRecord(record.id, record);
    };
    const snapshotPlatformZones = async () => await createSnapshot(zoneManager, { zoneKinds: ["platform"] });
    const startChangeSet = async (context) => {
    ...
    const recordValidationResults = async (results) => {
        const active = await loadActiveChangeSetRecord();
        if (!active || active.status !== "active") {
            return active;
        }
    ...
    const computeGitMetadata = async (paths) => {
        const gitRoot = await resolveGitRoot(zoneManager.projectRoot);
        if (!gitRoot) {
            return {
                gitHeadAtEnd: null,
    ...
    const evaluateDiffs = async (diffs, record, guardInput, gitNumstat) => {
        const changedFiles = [];
        const guardFailures = [];
        const blockReasons = [];
    ...
    const finishChangeSet = async (input) => {
        const active = await loadActiveChangeSetRecord();
        if (!active || active.status !== "active") {
            return {
                ok: false,
        ...
        const baselineSnapshot = await stateStore.loadChangeSetBaseline(active.id);
        if (!baselineSnapshot) {
            return {
                ok: false,
                status: "failed",
    ...
    const rollbackToLastKnownGood = async (reason) => {
        const baselineMetadata = await stateStore.loadBaselineMetadata();
        if (!baselineMetadata) {
            return { ok: false, reason: "No baseline metadata found." };
        }
        const snapshot = await stateStore.loadBaselineSnapshot(baselineMetadata.baselineId);
        if (!snapshot) {
    ...
    const rollbackChangeSet = async (changeSetId, reason) => {
        const record = await stateStore.loadChangeSetRecord(changeSetId);
        if (!record) {
            return { ok: false, reason: `ChangeSet not found: ${changeSetId}` };
        }
        const baselineSnapshot = await stateStore.loadChangeSetBaseline(changeSetId);
        if (!baselineSnapshot) {
