...
export function runMigrations(db) {
  ...
  `);
    const applied = new Set(db.prepare("SELECT version FROM _migrations").all()
        .map((r) => r.version));
    for (const migration of migrations) {
        ...
        const run = db.transaction(() => {
            migration.up(db);
            db.prepare("INSERT INTO _migrations (version, description, applied_at) VALUES (?, ?, ?)").run(migration.version, migration.description, Date.now());
        });
        run();
    }
}
