...
const parseUnlinkArgs = (args) => {
    const targets = args.filter((arg) => !arg.startsWith("-"));
    return { force: false, targets };
};
const hasPowerShellDeleteKeyword = (command) => /\b(?:Remove-Item|rm|del|erase|rd|rmdir)\b/i.test(command);
export const extractPowerShellDeleteTargets = (command) => {
    if (!hasPowerShellDeleteKeyword(command)) {
        return [];
    }
    const targets = [];
    const statementRegex = /\b(?:Remove-Item|rm|del|erase|rd|rmdir)\b([^;\n|]*)/gi;
...
export const extractPythonDeleteTargets = (code) => {
    const targets = [];
    const patterns = [
...
const runPassthrough = async (binary, args) => await new Promise((resolve) => {
...
const handleDeleteWrapper = async (args) => {
    const cwd = args[0];
    const tool = args[1];
    ...
    if (trashResult.trashed.length > 0) {
        console.log(`Moved ${trashResult.trashed.length} item(s) to Stella trash (auto-delete in 24h).`);
    }
    return trashResult.errors.length > 0 ? 1 : 0;
};
const findArgIndex = (args, ...needles) => {
...
const handlePowerShellWrapper = async (args) => {
    const cwd = args[0];
...
const handlePythonWrapper = async (args) => {
    const cwd = args[0];
...
export const runDeferredDeleteCli = async (argv) => {
    const mode = argv[0];
