...
const log = (...args) => console.log("[identity-map]", ...args);
 ...
 */
export function generateAlias(realIdentifier) {
    ...
    const lastNameIndex = Math.floor(hashInt / FIRST_NAMES.length) % LAST_NAMES.length;
 ...
 */
export async function loadIdentityMap(stellaHome) {
 ...
 */
export async function saveIdentityMap(stellaHome, map) {
 ...
 */
export async function addContacts(stellaHome, contacts) {
    const map = await loadIdentityMap(stellaHome);
    for (const contact of contacts) {
        // Check if mapping already exists for this identifier
        const existing = map.mappings.find((m) => m.real.identifier === contact.identifier);
        if (!existing) {
            // Generate new alias
            ...
            log(`Added mapping: ${contact.name} (${contact.identifier}) → ${alias.name} (${alias.identifier})`);
        }
    }
    // Save updated map
    ...
    for (const { real, alias, isIdentifier } of replacements) {
        if (isIdentifier) {
            ...
            if (digitsOnly.length >= 10) {
                ...
                const phonePattern = new RegExp(digitsOnly.split("").join("\\D*"), "g");
 ...
 */
export function depseudonymize(text, map) {
    if (!text || map.mappings.length === 0) {
        return text;
    }
