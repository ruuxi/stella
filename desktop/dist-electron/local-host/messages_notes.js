...
import os from "os";
const log = (...args) => console.log("[messages-notes]", ...args);
// Timeout wrapper
 ...
 */
async function collectIMessageMetadata(stellaHome) {
    ...
    try {
        ...
        LIMIT 30
      `;
            const contactRows = db.prepare(contactQuery).all();
        ...
        WHERE c.display_name IS NOT NULL
          ...
          AND c.style = 43
      `;
            const groupRows = db.prepare(groupQuery).all();
            const groupChats = groupRows.map((row) => ({
                name: row.name,
 ...
 * macOS only - requires Full Disk Access
 */
async function collectAppleNotes(stellaHome) {
    if (process.platform !== "darwin") {
        return [];
    }
    const sourceDb = path.join(os.homedir(), "Library/Group Containers/group.com.apple.notes/NoteStore.sqlite");
    const cacheDir = path.join(stellaHome, "cache");
 ...
 * macOS only
 */
async function collectReminders(stellaHome) {
    if (process.platform !== "darwin") {
        return [];
    }
    ...
    const possiblePaths = [
        ...
        path.join(os.homedir(), "Library/Group Containers/group.com.apple.reminders/Container_v1/Stores"),
    ];
    let sourceDb = null;
 ...
 * macOS only - requires Full Disk Access
 */
async function collectCalendar(stellaHome) {
    if (process.platform !== "darwin") {
        return [];
    }
    ...
    try {
      ...
      `;
            const calendarRows = db.prepare(calendarQuery).all();
        ...
        LIMIT 20
      `;
            const recurringRows = db.prepare(recurringQuery).all();
 ...
 * Windows only
 */
async function collectStickyNotes(stellaHome) {
 ...
 */
export async function collectMessagesNotes(stellaHome) {
 ...
 */
export function formatMessagesNotesForSynthesis(data) {
    const sections = [];
    // Communication Patterns
    if (data.contacts.length > 0) {
        const contactLines = data.contacts.map((c) => `- ${c.displayName} (${c.messageCount} messages)`);
