...
import { getDb, closeDb, newId, insert, update, remove, findById, query, rawQuery, rawRun, } from "./db";
import { handleChat } from "./agent/runtime";
const log = (...args) => console.log("[local-server]", ...args);
const logError = (...args) => console.error("[local-server]", ...args);
...
function broadcastSSE(conversationId, event, data) {
...
app.post("/api/conversations/default", async (c) => {
    ...
    const existing = rawQuery("SELECT * FROM conversations WHERE owner_id = ? AND is_default = 1 ORDER BY updated_at DESC LIMIT 1", [ownerId]);
...
app.patch("/api/conversations/:id", async (c) => {
    ...
    if ("tokenCount" in body)
        data.token_count = body.tokenCount;
...
app.get("/api/preferences/:key", (c) => {
    ...
    const ownerId = c.req.query("ownerId") || "local";
    const rows = rawQuery("SELECT * FROM user_preferences WHERE owner_id = ? AND key = ?", [ownerId, key]);
    if (rows.length === 0)
        return c.json(null);
...
app.put("/api/preferences/:key", async (c) => {
    ...
    const ownerId = body.ownerId || "local";
    const now = Date.now();
    const existing = rawQuery("SELECT id FROM user_preferences WHERE owner_id = ? AND key = ?", [ownerId, key]);
    if (existing.length > 0) {
...
app.get("/api/secrets", (c) => {
    ...
    const rows = rawQuery("SELECT id, owner_id, provider, label, key_version, status, metadata, created_at, updated_at, last_used_at FROM secrets WHERE owner_id = ?", [ownerId]);
    return c.json(rows);
});
app.post("/api/secrets", async (c) => {
    ...
    const now = Date.now();
    // Upsert by provider
    const existing = rawQuery("SELECT id FROM secrets WHERE owner_id = ? AND provider = ?", [ownerId, body.provider]);
    let secretId;
    if (existing.length > 0) {
...
app.get("/api/canvas/state", (c) => {
    ...
    if (!conversationId)
        return c.json(null);
    const rows = rawQuery("SELECT * FROM canvas_states WHERE owner_id = ? AND conversation_id = ?", [ownerId, conversationId]);
...
app.put("/api/canvas/state", async (c) => {
    ...
    const ownerId = body.ownerId || "local";
    const now = Date.now();
    const existing = rawQuery("SELECT id FROM canvas_states WHERE owner_id = ? AND conversation_id = ?", [ownerId, body.conversationId]);
    if (existing.length > 0) {
...
app.delete("/api/canvas/state", (c) => {
    ...
    const ownerId = c.req.query("ownerId") || "local";
    if (conversationId) {
        rawRun("DELETE FROM canvas_states WHERE owner_id = ? AND conversation_id = ?", [ownerId, conversationId]);
    }
    return c.json({ ok: true });
});
// ─── Skills ──────────────────────────────────────────────────────────────────
app.get("/api/skills", (c) => {
    ...
    if (enabledOnly) {
        return c.json(rawQuery("SELECT * FROM skills WHERE (owner_id = ? OR owner_id IS NULL) AND enabled = 1 ORDER BY updated_at DESC", [ownerId]));
    }
    return c.json(rawQuery("SELECT * FROM skills WHERE (owner_id = ? OR owner_id IS NULL) ORDER BY updated_at DESC", [ownerId]));
});
// ─── Agents ──────────────────────────────────────────────────────────────────
app.get("/api/agents/:type", (c) => {
    ...
    const rows = rawQuery("SELECT * FROM agents WHERE (owner_id = ? OR owner_id IS NULL) AND agent_types LIKE ? ORDER BY updated_at DESC LIMIT 1", [ownerId, `%"${agentType}"%`]);
    if (rows.length === 0)
        return c.json(null);
    return c.json(rows[0]);
});
...
app.post("/api/memories/recall", async (c) => {
    ...
    if (!body.embedding) {
        ...
        const rows = rawQuery("SELECT * FROM memories WHERE owner_id = ? ORDER BY accessed_at DESC LIMIT 10", [ownerId]);
        return c.json(rows);
    }
    // Vector search: brute-force cosine similarity in JS
    const allMemories = rawQuery("SELECT * FROM memories WHERE owner_id = ? AND embedding IS NOT NULL", [ownerId]);
...
app.post("/api/memories/save", async (c) => {
    ...
    if (body.embedding) {
        const existing = rawQuery("SELECT id, embedding FROM memories WHERE owner_id = ? AND embedding IS NOT NULL", [ownerId]);
        for (const mem of existing) {
            const memVec = typeof mem.embedding === "string"
                ? JSON.parse(mem.embedding)
                : mem.embedding;
            if (!Array.isArray(memVec))
                continue;
    ...
    const countResult = rawQuery("SELECT COUNT(*) as cnt FROM memories WHERE owner_id = ?", [ownerId]);
    if (countResult[0]?.cnt >= 500) {
        // Delete least-accessed
        rawRun("DELETE FROM memories WHERE id IN (SELECT id FROM memories WHERE owner_id = ? ORDER BY accessed_at ASC LIMIT 1)", [ownerId]);
    }
...
app.get("/api/store/packages", (c) => {
    ...
    if (search) {
        return c.json(rawQuery("SELECT * FROM store_packages WHERE search_text LIKE ? ORDER BY downloads DESC LIMIT 50", [`%${search}%`]));
    }
    if (type) {
        return c.json(rawQuery("SELECT * FROM store_packages WHERE type = ? ORDER BY downloads DESC LIMIT 50", [type]));
    }
    return c.json(rawQuery("SELECT * FROM store_packages ORDER BY downloads DESC LIMIT 50", []));
});
app.get("/api/store/installed", (c) => {
    const ownerId = c.req.query("ownerId") || "local";
    const rows = rawQuery(`SELECT si.*, sp.name, sp.type, sp.description, sp.icon
     FROM store_installs si
     LEFT JOIN store_packages sp ON si.package_id = sp.package_id
     WHERE si.owner_id = ?
     ORDER BY si.installed_at DESC`, [ownerId]);
    return c.json(rows);
});
app.post("/api/store/install", async (c) => {
    ...
    const now = Date.now();
    // Upsert
    const existing = rawQuery("SELECT id FROM store_installs WHERE owner_id = ? AND package_id = ?", [ownerId, body.packageId]);
    if (existing.length > 0) {
...
app.post("/api/store/uninstall", async (c) => {
    const body = await c.req.json();
    const ownerId = body.ownerId || "local";
    rawRun("DELETE FROM store_installs WHERE owner_id = ? AND package_id = ?", [ownerId, body.packageId]);
    return c.json({ ok: true });
});
// ─── STT ─────────────────────────────────────────────────────────────────────
...
app.get("/api/usage", (c) => {
    ...
    const rows = rawQuery("SELECT * FROM usage_logs WHERE owner_id = ? ORDER BY created_at DESC LIMIT ?", [ownerId, limit]);
    return c.json(rows);
});
// ─── Commands ────────────────────────────────────────────────────────────────
...
app.post("/api/synthesize", async (c) => {
    ...
    try {
        ...
        if (!response.ok) {
            return c.json({ error: `Synthesis proxy error: ${response.status}` }, response.status);
        }
...
export function startLocalServer(port = DEFAULT_PORT) {
