 ...
 */
const log = (...args) => console.log("[signal-processing]", ...args);
// ---------------------------------------------------------------------------
 ...
 */
export function filterLowSignalDomains(formatted) {
    ...
    for (const header of sectionHeaders) {
        ...
        result = result.replace(regex, (_fullMatch, hdr, body) => {
 ...
 */
export function tierFormattedSignals(formatted) {
    const sections = [];
    ...
    const result = [];
    if (tier1.length > 0)
        result.push("# Tier 1: Core Signals (highest priority for synthesis)\n\n" + join(tier1));
    if (tier2.length > 0)
        result.push("# Tier 2: Supporting Context\n\n" + join(tier2));
    if (tier3.length > 0)
        result.push("# Tier 3: Supplementary\n\n" + join(tier3));
    return result.join("\n\n");
}
