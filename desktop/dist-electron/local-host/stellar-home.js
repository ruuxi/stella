...
const migrateDirectoryFiles = async (fromDir, toDir) => {
    let migratedCount = 0;
    let entries = [];
    try {
...
const migrateFromUserData = async (userDataPath, stellarHomePath) => {
    ...
    const deviceMigrated = await copyIfMissing(path.join(userDataPath, "device.json"), path.join(statePath, "device.json"));
...
export const resolveStellarHome = async (app, userDataPath) => {
