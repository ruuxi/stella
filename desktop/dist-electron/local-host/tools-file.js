...
export function setFileToolsConfig(config) {
    _config = config;
}
/**
...
const createAndActivateFeature = async (context, relativePath, reason) => {
    const featureId = `mod-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    const topic = normalizeTopic(relativePath);
    await createFeature(featureId, buildFeatureName(relativePath), topic
        ? `Auto-created self-mod feature (${reason}) for ${topic}`
        : `Auto-created self-mod feature (${reason})`, context.conversationId);
...
async function ensureActiveFeature(context, relativePath) {
    ...
    if (stagedFiles.length > 0) {
        ...
        if (now - feature.updatedAt > FEATURE_TOPIC_SHIFT_MS) {
            return createAndActivateFeature(context, relativePath, "staged changes are from a different topic");
        }
        return featureId;
    }
    ...
    if (lastBatchFiles.length > 0 &&
        !isRelatedPath(relativePath, lastBatchFiles) &&
        idleForMs > FEATURE_TOPIC_SHIFT_MS) {
        return createAndActivateFeature(context, relativePath, "topic shift detected");
    }
    return featureId;
}
export const handleRead = async (args, context) => {
...
export const handleWrite = async (args, context) => {
    const filePath = expandHomePath(String(args.file_path ?? ""));
...
export const handleEdit = async (args, context) => {
    const filePath = expandHomePath(String(args.file_path ?? ""));
