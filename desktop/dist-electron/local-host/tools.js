...
import { createSnapshot, diffSnapshots, restoreSnapshot, } from "./core-host/snapshots.js";
import { ensureWithinRoot } from "./core-host/path-utils.js";
import { defaultValidationSpecs, runValidations, } from "./core-host/validations.js";
...
export const createToolHost = ({ stellarHome, projectRoot, deviceId, screenBridge, }) => {
    ...
    const getAgentType = (context) => context.agentType ?? "general";
    const isSelfMod = (context) => getAgentType(context) === "self_mod";
    ...
    const resolvePathForRead = (filePath) => {
    ...
    const resolvePathForWrite = async (filePath, context, options) => {
        const resolved = zoneManager.resolvePath(filePath);
        if (!resolved.ok) {
            return resolved;
        }
    ...
    const handleWrite = async (args, context) => {
        const filePath = String(args.file_path ?? "");
    ...
    const handleEdit = async (args, context) => {
        const filePath = String(args.file_path ?? "");
    ...
    const handleBash = async (args, context) => {
        ...
        const timeout = Math.min(Math.max(Number(args.timeout ?? 120000), 5000), 600000);
        const runInBackground = Boolean(args.run_in_background ?? false);
        const userConfirmed = Boolean(args.user_confirmed ?? false);
        const overrideGuard = Boolean(args.override_guard ?? false);
        if (!isSelfMod(context) && runInBackground) {
            return {
                error: "Background shell execution is restricted to the Self-Modification agent to preserve rollback safety.",
            };
        }
        ...
        if (platformBaseline) {
            ...
            if (diffs.length > 0) {
                ...
                return {
                    error: "Command attempted to modify platform zones and was rolled back.\n\nChanged paths:\n" +
                        changed,
                };
            }
        }
    ...
    const parseValidationSpecs = (checks) => {
        const specs = [];
    ...
    const handleValidationRun = async (args, _context) => {
        void _context;
    ...
    const handleChangeSetFinish = async (args, context) => {
        void context;
    ...
    const handleChangeSetRollback = async (args) => {
    ...
    const handleChangeSetStatus = async () => {
    ...
    const handlePackPublish = async (args, context) => {
        const userConfirmed = Boolean(args.user_confirmed ?? false);
        if (!userConfirmed) {
    ...
    const handlePackInstall = async (args, context) => {
    ...
    const handlePackUninstall = async (args, context) => {
        const packId = String(args.pack_id ?? "").trim();
    ...
    const handleUpdateCheck = async (args, _context) => {
        void _context;
    ...
    const handleUpdateApply = async (args, context) => {
        const channelId = String(args.channel_id ?? "").trim();
    ...
    const handleScreenInvoke = async (args, context) => {
        if (!screenBridge) {
    ...
    const handleScreenList = async (_args, context) => {
        ...
        try {
            ...
            if (isPluginTool && pluginBaseline) {
                ...
                if (diffs.length > 0) {
                    ...
                    const changed = diffs.slice(0, 8).map((diff) => diff.virtualPath).join("\n");
                    return {
                        error: "Plugin tool attempted to modify platform zones and was rolled back.\n\nChanged paths:\n" +
                            changed,
                    };
                }
            }
