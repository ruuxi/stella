...
export const handleSelfModStart = async (args, context, frontendRoot) => {
    ...
    const meta = await createFeature(featureId, name, description, context.conversationId);
...
export const handleSelfModApply = async (args, context, frontendRoot) => {
    if (!frontendRoot) {
    ...
    try {
        ...
        return { error: `Failed to apply: ${error.message}` };
    }
};
export const handleSelfModRevert = async (args, context, frontendRoot) => {
    if (!frontendRoot) {
    ...
    const remainingHistory = await removeLastHistoryEntries(featureId, batchesToRevert);
...
export const handleSelfModStatus = async (args, context) => {
    const featureId = args.feature_id
        ? String(args.feature_id)
        : await getActiveFeature(context.conversationId);
    if (!featureId) {
...
export const handleSelfModPackage = async (args, context, frontendRoot) => {
    if (!frontendRoot) {
    ...
    try {
        ...
        return { error: `Failed to package: ${error.message}` };
    }
};
