...
import { packageJsonTemplate, viteConfigTemplate, indexHtmlTemplate, mainTsxTemplate, appTsxTemplate, tsconfigTemplate, } from './workspace_templates.js';
const workspaces = new Map();
const processes = new Map();
const getPrimaryWorkspacesRoot = () => process.env.STELLA_WORKSPACES_ROOT?.trim() || path.join(os.homedir(), 'workspaces');
...
const findExistingWorkspacePath = async (workspaceId) => {
...
const toId = (name) => name.toLowerCase().replace(/[^a-z0-9_-]/g, '-');
/**
...
export const handleCreateWorkspace = async (args) => {
    ...
    try {
        ...
        await Promise.all([
            ...
            fs.writeFile(path.join(wsPath, 'src', 'App.tsx'), typeof args.source === 'string' ? args.source : appTsxTemplate(name)),
        ]);
        ...
        await new Promise((resolve, reject) => {
            ...
            child.on('close', (code) => {
                if (code === 0)
                    ...
                    reject(new Error(`bun install failed (exit ${code}):\n${output}`));
            });
...
export const handleStartDevServer = async (args) => {
    ...
    try {
        ...
        return { error: `Failed to start dev server: ${err.message}` };
    }
};
export const handleStopDevServer = async (args) => {
    const id = String(args.workspaceId ?? '').trim();
    if (!id)
        return { error: 'workspaceId is required.' };
