...
If `secretMounts` is omitted, Stellar will attempt to infer env vars and token file paths from the markdown as a best-effort convenience.
If `secretMounts` is provided (even partially), Stellar will use it as the source of truth and skip inference.
