...
export const registerScheduleHandlers = (options: ScheduleHandlersOptions) => {
  ...
  ipcMain.handle(
    ...
    (event, payload: { conversationId?: string; maxItems?: number }) => {
      if (!options.assertPrivilegedSender(event, 'schedule:listConversationEvents')) {
        throw new Error('Blocked untrusted schedule:listConversationEvents request.')
      }
  ...
  ipcMain.handle(
    ...
    (event, payload: { conversationId?: string }) => {
      if (!options.assertPrivilegedSender(event, 'schedule:getConversationEventCount')) {
        throw new Error('Blocked untrusted schedule:getConversationEventCount request.')
      }
      const conversationId =
        typeof payload?.conversationId === 'string' ? payload.conversationId.trim() : ''
      if (!conversationId) {
