...
export const registerMiniBridgeHandlers = (
  options: MiniBridgeHandlersOptions,
) => {
  ipcMain.handle(
    "miniBridge:request",
    async (event, request: MiniBridgeRequest) => {
  ...
  ipcMain.on(
    "miniBridge:response",
    (event, envelope: MiniBridgeResponseEnvelope) => {
