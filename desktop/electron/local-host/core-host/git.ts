...
export const checkGitAvailable = async (cwd: string) => {
  const result = await runCommand("git", ["--version"], cwd, GIT_TIMEOUT_MS);
  return result.ok;
};

export const resolveGitRoot = async (cwd: string) => {
...
export const getGitHead = async (cwd: string) => {
...
export const getGitNumStat = async (cwd: string, paths: string[] = []) => {
  const cleanPaths = sanitizePaths(paths);
  const args =
    cleanPaths.length > 0
...
export const getGitChangedPaths = async (cwd: string, paths: string[] = []) => {
  const cleanPaths = sanitizePaths(paths);
  const args =
    cleanPaths.length > 0
...
export const applyReversePatch = async (cwd: string, patchContent: string) => {
  ...
  try {
    ...
    const result = await runCommand(
      "git",
      ["apply", "-R", "--whitespace=nowarn", tempFile],
      cwd,
      60_000,
    );
    return {
