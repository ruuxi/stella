...
import type { ChangeSetManager, ChangeSetRecord, ConvexBridge } from "./change-sets.js";
...
export type PackSecurityReview = {
...
export type PackManifest = {
  ...
  securityReview: PackSecurityReview;
  bundleHash: string;
  signature: string;
};
...
export type PackInstallationRecord = {
  installId: string;
  packId: string;
  name: string;
  description: string;
  version: string;
  status: "installed" | "failed" | "uninstalled" | "disabled_safe_mode";
  installedAt: number;
  updatedAt: number;
  deviceId: string;
  bundleHash: string;
...
export type UninstallPackInput = {
  packId: string;
  version?: string;
  deviceId: string;
  conversationId?: string;
...
const PACK_DIFF_LIMIT = 300_000;
const truncateDiff = (value: string) => {
...
const sanitizePackId = (value: string) => {
  const base = value
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
...
const isSemverLike = (version: string) => /^\d+\.\d+\.\d+([-+][0-9A-Za-z.-]+)?$/.test(version);
...
const buildBundlePath = (packsRoot: string, packId: string, version: string) => {
  return path.join(packsRoot, "bundles", packId, `${version}.bundle.json`);
};

const ensureBundleDir = async (packsRoot: string, packId: string) => {
  const dir = path.join(packsRoot, "bundles", packId);
  await fs.mkdir(dir, { recursive: true });
  return dir;
};

const collectChangeSets = async (stateStore: StateStore, changeSetIds: string[]) => {
  const records: ChangeSetRecord[] = [];
  for (const id of changeSetIds) {
    const record = await stateStore.loadChangeSetRecord<ChangeSetRecord>(id);
    if (!record) {
      return { ok: false as const, reason: `ChangeSet not found: ${id}` };
    }
    if (record.status !== "completed") {
      return { ok: false as const, reason: `ChangeSet is not completed: ${id}` };
    }
    records.push(record);
  }
  records.sort((a, b) => {
...
const collectChangedPaths = (records: ChangeSetRecord[]) => {
  const paths = new Set<string>();
  const zones = new Set<string>();
  const compatibility = new Set<string>();
  const actionMap = new Map<string, PackEntry["action"]>();
...
const buildEntriesFromPaths = async (
  zoneManager: ZoneManager,
  changedPaths: string[],
  actionMap: Map<string, PackEntry["action"]>,
): Promise<PackEntry[]> => {
  ...
  for (const virtualPath of changedPaths) {
    ...
    try {
      ...
      if (stat.isFile()) {
        const entry = await readFileAsEntry(
          absolutePath,
          classification.virtualPath,
          zone,
          projectRelativePath,
        );
...
const verifyBundleSignature = (bundle: PackBundle) => {
  ...
  const signatureValid = verifySignature(
    bundle.manifest.authorPublicKey,
    hashed.hashHex,
...
export const createPackManager = (options: PackManagerOptions) => {
  const { zoneManager, stateStore, changeSetManager, instructionManager, deviceId: hostDeviceId } =
    options;
  ...
  const callAction = async (name: string, args: Record<string, unknown>) => {
    if (!convexBridge?.callAction) return null;
    try {
  ...
  const listInstallations = async () =>
    await stateStore.loadPackInstallations<PackInstallationRecord>();

  const saveInstallations = async (installations: PackInstallationRecord[]) => {
    await stateStore.savePackInstallations(installations);
  };

  const publishPack = async (input: PublishPackInput): Promise<PublishPackResult> => {
    ...
    const entries = await buildEntriesFromPaths(
      zoneManager,
    ...
    const securityReviewPayload = {
      ...
      entries: entries.map((entry) => ({
        ...
        hash: entry.hash,
        preview:
          entry.content && entry.encoding === "utf8"
            ? entry.content.slice(0, 20_000)
            : undefined,
      })),
    ...
    })) as PackSecurityReview | null;

    const securityReview: PackSecurityReview = securityReviewResult ?? {
      status: "needs_changes",
      summary:
        "Security review could not be completed by the backend. Submission is blocked until review succeeds.",
  ...
  const fetchPackBundle = async (packId: string, version: string) => {
    ...
    return { bundle: remote.bundle, source: "remote" as const };
  };

  const applyEntry = async (entry: PackEntry) => {
  ...
  const installPack = async (input: InstallPackInput): Promise<InstallPackResult> => {
    await stateStore.ensureStructure();
    await changeSetManager.ensureBaseline();
    ...
    try {
      ...
      return {
        ok: false,
        reason: `Failed to apply pack entries after ${appliedCount} changes: ${
    ...
    const nextInstallations = [
      ...
      ...installations.filter(
        (item) => !(item.packId === record.packId && item.version === record.version),
      ),
    ];
  ...
  const findInstallation = async (packId: string, version?: string) => {
    const installations = await listInstallations();
    const candidates = installations.filter(
      (item) => item.packId === packId && (!version || item.version === version),
    );
  ...
  const uninstallPack = async (input: UninstallPackInput): Promise<UninstallPackResult> => {
    await stateStore.ensureStructure();
    await changeSetManager.ensureBaseline();
    ...
    const uninstallSnapshot = await stateStore.loadPackUninstallSnapshot<Snapshot>(
    ...
    const changedPaths =
      installation.changedPaths && installation.changedPaths.length > 0
  ...
  const disableAllForSafeMode = async (reason: string) => {
    const installations = await listInstallations();
    ...
    const next = installations.map((item) => {
      ...
      return {
        ...item,
        status: "disabled_safe_mode" as const,
        updatedAt: now,
        lastError: reason,
      };
    });
