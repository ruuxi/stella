...
import type { ChangeSetManager, ConvexBridge } from "./change-sets.js";
import type { InstructionManager } from "./instructions.js";
import type { PackBundle, PackEntry } from "./packs.js";
import type { StateStore } from "./state-store.js";
import type { Snapshot, SnapshotFile } from "./snapshots.js";
...
const contentToBuffer = (encoding: "utf8" | "base64" | undefined, content: string | undefined) => {
...
const snapshotFileToBuffer = (file: SnapshotFile | undefined) => {
  if (!file) return null;
...
const buffersEqual = (a: Buffer | null, b: Buffer | null) => {
  if (!a && !b) return true;
  if (!a || !b) return false;
  return a.equals(b);
};

const buildLocalFile = (file: SnapshotFile | undefined) => {
  if (!file) {
    return undefined;
  }
  return {
...
const buildUpstreamFile = (entry: PackEntry) => {
...
const selectBaseSnapshot = async (
  stateStore: StateStore,
  baseGitHead: string | null | undefined,
) => {
  ...
  if (!baseGitHead) {
    ...
    const snapshot = await stateStore.loadBaselineSnapshot<Snapshot>(currentBaseline.baselineId);
    if (!snapshot) {
      return null;
    }
  ...
  if (match) {
    const snapshot = await stateStore.loadBaselineSnapshot<Snapshot>(match.baselineId);
    if (snapshot) {
...
const computeConflicts = async (
  ...
  for (const entry of bundle.entries) {
    ...
    const upstreamBuffer =
      entry.action === "delete" ? null : contentToBuffer(entry.encoding, entry.content);

    const localChanged = !buffersEqual(baseBuffer, localBuffer);
    const upstreamChanged =
      entry.action === "delete" ? Boolean(baseFile) : !buffersEqual(baseBuffer, upstreamBuffer);
...
const applyEntry = async (zoneManager: ZoneManager, entry: PackEntry) => {
...
const mergeConflicts = async (
  ...
  if (conflicts.length === 0) {
    return { ok: true as const, resolutions: [] as MergeResolution[] };
  }
  ...
  const payload = {
    ...
    agentType: "self_mod",
    prompt:
      "Resolve semantic merge conflicts between upstream and local changes. Follow the provided rules and return JSON only.",
    input: {
      rules: [
  ...
  const result = (await convexBridge.callAction("agent.invoke", payload)) as
    | { ok?: boolean; resolutions?: MergeResolution[]; reason?: string }
    | null;
...
export const createUpdateManager = (options: UpdateManagerOptions) => {
  ...
  const checkForUpdates = async (channelId: string): Promise<UpdateCheckResult> => {
    const response = (await callAction("updates.getLatestRelease", {
      channelId,
    })) as
      | { ok?: boolean; release?: UpstreamRelease; reason?: string; channelId?: string }
      | null;
  ...
  const applyUpdate = async (input: UpdateApplyInput): Promise<UpdateApplyResult> => {
    ...
    const releaseResponse = (await callAction("updates.getReleaseForApply", {
      ...
      | { ok?: boolean; release?: UpstreamRelease; reason?: string }
      | null;
    ...
    const conflictResult = await computeConflicts(
      release.bundle,
      base.snapshot,
      currentSnapshot,
      instructionManager,
    ...
    try {
      ...
      for (const conflict of conflictResult.conflicts) {
        ...
        if (resolution.strategy === "use_upstream") {
          const upstreamEntry = release.bundle.entries.find(
            (entry) => entry.virtualPath === conflict.virtualPath,
          );
          if (!upstreamEntry) {
      ...
      return {
        ...
        reason: `Failed to apply update: ${(error as Error).message}`,
        conflicts: conflictResult.conflicts.length,
      };
    }
    ...
    const appliedHistory = await (async () => {
      try {
        ...
        return JSON.parse(raw) as typeof appliedRecord[];
      } catch {
        return [] as typeof appliedRecord[];
      }
    })();
    await fs.mkdir(path.dirname(appliedPath), { recursive: true });
    await fs.writeFile(
      appliedPath,
      JSON.stringify([appliedRecord, ...appliedHistory].slice(0, 50), null, 2),
      "utf-8",
    );
