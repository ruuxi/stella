...
export type ZoneKind = "platform" | "user";
...
type CreateZoneManagerOptions = {
  projectRoot: string;
  stellarHome: string;
};

export type ZoneGuardContext = {
  agentType: string;
  operation:
    | "self_mod"
    | "pack_publish"
...
export type ZoneGuardResult = {
  ok: boolean;
  reason?: string;
};

const getZoneRoots = (projectRoot: string, stellarHome: string) => {
...
const buildZones = (projectRoot: string, stellarHome: string): ZoneDefinition[] => {
...
const pickBestZone = (zones: ZoneDefinition[], absolutePath: string) => {
  const matches: Array<{ zone: ZoneDefinition; root: string }> = [];
...
const virtualToAbsolute = (zones: ZoneDefinition[], virtualPath: string) => {
...
const classifyPathInternal = (
  ...
  const zoneRelativePath =
    zone && zoneRoot ? relativeToRoot(zoneRoot, normalizedAbsolute) : toPosix(normalizedAbsolute);
...
const guardPlatformZone = (zone: ZoneDefinition, context: ZoneGuardContext): ZoneGuardResult => {
  ...
  const needsSelfMod =
    "Platform zones may only be modified by the Self-Modification agent (or user-confirmed system operations).";
  return {
    ok: false,
...
const guardUserZone = (_zone: ZoneDefinition, context: ZoneGuardContext): ZoneGuardResult => {
...
const guardUnknownZone = (context: ZoneGuardContext): ZoneGuardResult => {
  ...
  return {
    ok: false,
    reason:
      "Path is outside all known zones. Refuse to modify it unless explicitly routed through a user-confirmed system operation.",
  };
};

export const createZoneManager = (options: CreateZoneManagerOptions) => {
  ...
  const classifyPath = (inputPath: string): ZoneClassification => {
  ...
  const enforceGuard = (
    inputPath: string,
    context: ZoneGuardContext,
  ): ZoneGuardResult & { classification: ZoneClassification } => {
    ...
    const result =
      zone.kind === "platform" ? guardPlatformZone(zone, context) : guardUserZone(zone, context);
    return { ...result, classification };
  };
...
export type ZoneManager = ReturnType<typeof createZoneManager>;
