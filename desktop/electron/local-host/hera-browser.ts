...
const log = (...args: unknown[]) => console.log("[hera-browser]", ...args);
const logError = (...args: unknown[]) => console.error("[hera-browser]", ...args);
...
const handleExecute = async (args: Record<string, unknown>): Promise<ToolResult> => {
...
const handleReset = async (): Promise<ToolResult> => {
  log("Reset requested");
...
export const HERA_BROWSER_TOOL_DESCRIPTORS: PluginToolDescriptor[] = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        code: {
          type: "string",
          description:
            "Playwright code to execute. Has {page, state, context} in scope. Use semicolons for multiple statements.",
        },
        timeout: {
          type: "number",
...
export const HERA_BROWSER_PLUGIN: PluginRecord = {
  id: "hera-browser",
  name: "Hera Browser",
  version: "0.0.56",
  description:
    "Control Chrome browser via Playwright. Automate web interactions, take screenshots, and inspect accessibility trees.",
  source: "builtin",
};
...
export const HERA_BROWSER_SKILL = {
  ...
  name: "Hera Browser",
  description:
    "Control Chrome browser via Playwright code snippets. Automate web interactions, take screenshots, inspect accessibility trees, and debug web applications.",
