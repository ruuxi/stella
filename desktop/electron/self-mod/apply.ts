...
const renameWithDir = async (fromPath: string, toPath: string) => {
...
export async function applyBatch(
  ...
  try {
    ...
    for (const op of [...operations].reverse()) {
      if (!op.installed) continue;
      try {
        await removeIfExists(op.sourcePath);
      } catch (cleanupError) {
        rollbackErrors.push(
          `Failed removing partial ${op.relativePath}: ${(cleanupError as Error).message}`,
        );
      }
    }
    ...
    for (const op of [...operations].reverse()) {
      if (!op.backupCreated) continue;
      try {
        await renameWithDir(op.backupPath, op.sourcePath);
      } catch (restoreError) {
        rollbackErrors.push(
          `Failed restoring backup ${op.relativePath}: ${(restoreError as Error).message}`,
        );
      }
    }
    ...
    const baseMessage = `Atomic apply failed: ${(error as Error).message}`;
 ...
 */
export async function removeLastHistoryEntries(
  featureId: string,
  count: number,
