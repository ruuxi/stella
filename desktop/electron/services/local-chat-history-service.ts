...
type LocalChatHistoryServiceOptions = {
  ...
  /**
   * Fired when the mirrored task status decoration changes. Wired to a
   * mobile-only broadcast — desktop windows
  ...
  listAgentThreadMessages(args: { threadId: string; limit?: number }) {
    ...
    const authored = new Set(
      projected
        ...
        .map((message) => `${message.role}\0${message.content.trim()}`),
    );
    ...
    if (agentRecord?.parentAgentId) {
      ...
      if (parentRecord?.conversationId === agentRecord.conversationId) {
        const parentRows = store.loadRawThreadMessagesWithEntryTypes(
          agentRecord.parentAgentId,
        );
