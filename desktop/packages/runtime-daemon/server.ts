...
import { ensurePrivateDirSync, writePrivateFileSync } from "../runtime-kernel/home/private-fs.js";
...
export const createRuntimeDaemonServer = (peer: JsonRpcPeer) => {
  ...
  const spawnWorker = async () => {
    ...
    const { peer: workerPeer } = attachJsonRpcPeerToStreams({
      ...
      output: child.stdin,
      onError: (error) => {
        console.error("[runtime-daemon] worker RPC error:", error);
      },
    });
    ...
    workerPeer.registerNotificationHandler(NOTIFICATION_NAMES.VOICE_AGENT_EVENT, (params) => {
      peer.notify(NOTIFICATION_NAMES.VOICE_AGENT_EVENT, params);
    });
    workerPeer.registerNotificationHandler(
      NOTIFICATION_NAMES.VOICE_SELF_MOD_HMR_STATE,
      (params) => {
        peer.notify(NOTIFICATION_NAMES.VOICE_SELF_MOD_HMR_STATE, params);
      },
    );
    ...
    workerPeer.registerNotificationHandler(NOTIFICATION_NAMES.PROJECTS_UPDATED, (params) => {
      peer.notify(NOTIFICATION_NAMES.PROJECTS_UPDATED, params);
    });
    workerPeer.registerNotificationHandler(
      NOTIFICATION_NAMES.OVERLAY_AUTO_PANEL_EVENT,
      (params) => {
        peer.notify(NOTIFICATION_NAMES.OVERLAY_AUTO_PANEL_EVENT, params);
      },
    );
        ...
        setTimeout(() => {
          if (state.init && !state.worker) {
            void spawnWorker().catch((error) => {
              console.error("[runtime-daemon] Failed to respawn worker:", error);
            });
          }
        }, 250).unref();
      }
  ...
  const workerProxyMethods = [
    ...
    METHOD_NAMES.VOICE_ORCHESTRATOR_CHAT,
    METHOD_NAMES.VOICE_WEB_SEARCH,
    ...
    METHOD_NAMES.PROJECTS_LIST,
    METHOD_NAMES.PROJECTS_REGISTER_DIRECTORY,
    METHOD_NAMES.PROJECTS_START,
    METHOD_NAMES.PROJECTS_STOP,
    METHOD_NAMES.OVERLAY_AUTO_PANEL_START,
    METHOD_NAMES.OVERLAY_AUTO_PANEL_CANCEL,
    METHOD_NAMES.DASHBOARD_START_PERSONAL_WEBSITE_GENERATION,
    METHOD_NAMES.SELF_MOD_REVERT,
    METHOD_NAMES.SELF_MOD_LAST_FEATURE,
  ...
  const workerMethodMap: Record<(typeof workerProxyMethods)[number], string> = {
    ...
    [METHOD_NAMES.PROJECTS_LIST]: METHOD_NAMES.INTERNAL_WORKER_PROJECTS_LIST,
    [METHOD_NAMES.PROJECTS_REGISTER_DIRECTORY]:
      METHOD_NAMES.INTERNAL_WORKER_PROJECTS_REGISTER_DIRECTORY,
    [METHOD_NAMES.PROJECTS_START]: METHOD_NAMES.INTERNAL_WORKER_PROJECTS_START,
    [METHOD_NAMES.PROJECTS_STOP]: METHOD_NAMES.INTERNAL_WORKER_PROJECTS_STOP,
    [METHOD_NAMES.OVERLAY_AUTO_PANEL_START]:
    ...
    [METHOD_NAMES.SELF_MOD_REVERT]: METHOD_NAMES.INTERNAL_WORKER_SELF_MOD_REVERT,
    [METHOD_NAMES.SELF_MOD_LAST_FEATURE]:
