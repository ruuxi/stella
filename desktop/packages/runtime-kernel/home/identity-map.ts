 ...
 * never sees real PII. The mapping persists at install-root
 * `.stella/state/identity_map.json`.
 */
...
export async function addContacts(
  ...
  for (const contact of contacts) {
    ...
    if (!existing) {
      ...
      log(
        `Added mapping: ${contact.name} (${contact.identifier}) Ã¢â€ â€™ ${alias.name} (${alias.identifier})`
      );
    }
  }
...
export function pseudonymize(text: string, map: IdentityMap): string {
  ...
  // Build replacement pairs: real Ã¢â€ â€™ alias
...
export function depseudonymize(text: string, map: IdentityMap): string {
  ...
  // Build replacement pairs: alias Ã¢â€ â€™ real
  const replacements: Array<{
    alias: string;
    real: string;
    isIdentifier: boolean;
