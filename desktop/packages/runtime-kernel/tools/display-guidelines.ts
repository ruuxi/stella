// Adapted from claude.ai's generative UI guidelines for Stella's Display tool.

const CORE = `# Display — Visual Creation Suite
...
You create rich visual content — SVG diagrams/illustrations and HTML interactive widgets — that renders inline in a dashboard panel. The best output feels like a natural extension of the interface.
...
### Philosophy
- **Seamless**: Users shouldn't notice where the app ends and your display begins.
- **Flat**: No gradients, mesh backgrounds, noise textures, or decorative effects. Clean flat surfaces.
- **Compact**: Show the essential inline. Explain the rest in text.
- **Text goes in your response, visuals go in the tool** — All explanatory text, descriptions, introductions, and summaries must be written as normal response text OUTSIDE the tool call. The tool output should contain ONLY the visual element (diagram, chart, interactive widget). Never put paragraphs of explanation, section headings, or descriptive prose inside the HTML/SVG. If the user asks "explain X", write the explanation in your response and use the tool only for the visual that accompanies it. The user's font settings only apply to your response text, not to text inside the display.

### Streaming
Output streams token-by-token via DOM-diffing with fade-in animations. Structure code so useful content appears early.
- **HTML**: \`<style>\` (short) → content HTML → \`<script>\` last.
- **SVG**: \`<defs>\` (markers) → visual elements immediately.
- Prefer inline \`style="..."\` over \`<style>\` blocks — inputs/controls must look correct mid-stream.
- Keep \`<style>\` under ~15 lines. Interactive widgets with inputs and sliders need more style rules — that's fine, but don't bloat with decorative CSS.
- Gradients, shadows, and blur flash during streaming DOM diffs. Use solid flat fills instead.
- Content streams via morphdom-style DOM diffing — avoid gradients/shadows, avoid display:none during streaming.
...
- No dark/colored backgrounds on outer containers (transparent only — the \`.canvas-display\` container provides the bg)
- **Typography**: The default font is Inter (inherited). For the rare editorial/blockquote moment, use \`font-family: Georgia, serif\`. For code, use \`font-family: var(--font-family-mono, monospace)\`.
- **Headings**: h2 = 19-22px, h3 = 10px uppercase — all \`font-weight: 500\`. Heading color uses \`var(--foreground)\` with \`opacity: 0.92\` — don't override it. Body text = 12.5-13px, weight 400, \`line-height: 1.7\`. **Two weights only: 400 regular, 500 bold.** Never use 600 or 700 — they look heavy against the host UI.
- **Sentence case** always. Never Title Case, never ALL CAPS. This applies everywhere including SVG text labels and diagram headings.
- **No mid-sentence bolding**, including in your response text around the tool call. Entity names, class names, function names go in \`code style\` not **bold**. Bold is for headings and labels only.
- The display container is \`display: block; width: 100%\`. Your HTML fills it naturally — no wrapper div needed. Just start with your content directly. If you want vertical breathing room, add \`padding: 1rem 0\` on your first element.
- Never use \`position: fixed\` — the container sizes itself to your in-flow content height, so fixed-positioned elements (modals, overlays, tooltips) collapse it. For modal/overlay mockups: wrap everything in a normal-flow \`<div style="min-height: 400px; background: rgba(0,0,0,0.45); display: flex; align-items: center; justify-content: center;">\` and put the modal inside — it's a faux viewport that actually contributes layout height.
- No DOCTYPE, \`<html>\`, \`<head>\`, or \`<body>\` — just content fragments.
- When placing text on a colored background (badges, pills, cards, tags), use the darkest shade from that same color family for the text — never plain black or generic gray.
- **Corners**: use \`border-radius: 8px\` (or \`12px\` for cards) in HTML. In SVG, \`rx="4"\` is the default — larger values make pills, use only when you mean a pill.
- **No rounded corners on single-sided borders** — if using \`border-left\` or \`border-top\` accents, set \`border-radius: 0\`. Rounded corners only work with full borders on all sides.
- **No titles or prose inside the tool output** — see Philosophy above.
- **Icon sizing**: When using inline SVG icons, explicitly set \`width: 16px; height: 16px\` for SVG icons. Never let icons inherit the container's font size — they will render too large. For larger decorative icons, use 24px max.
- No tabs, carousels, or \`display: none\` sections during streaming — hidden content streams invisibly. Show all content stacked vertically. (Post-streaming JS-driven steppers are fine — see Illustrative/Interactive sections.)
- No nested scrolling — auto-fit height.
- Scripts ARE allowed and execute after the full HTML is set. Load libraries via \`<script src="https://cdnjs.cloudflare.com/ajax/libs/...">\` (UMD globals), then use the global in a plain \`<script>\` that follows.
- **CDN allowlist**: external resources may ONLY load from \`cdnjs.cloudflare.com\`, \`esm.sh\`, \`cdn.jsdelivr.net\`, \`unpkg.com\`.

### CSS Variables
**Backgrounds**: \`var(--background)\` (primary), \`color-mix(in oklch, var(--foreground) 3%, transparent)\` (surfaces)
**Text**: \`var(--foreground)\` with opacity tiers — 0.92 (title/primary), 0.88 (key values), 0.65 (body/secondary), 0.42 (tertiary/hints), 0.25-0.3 (meta)
**Borders**: \`color-mix(in oklch, var(--foreground) 7%, transparent)\` (default), \`color-mix(in oklch, var(--foreground) 12%, transparent)\` (hover/secondary), \`color-mix(in oklch, var(--foreground) 18%, transparent)\` (primary/strong)
**Semantic colors**: Use hardcoded oklch values for info, danger, success, warning
**Typography**: Inter (inherited sans), \`Georgia, serif\` (serif), \`var(--font-family-mono, monospace)\` (code)
**Layout**: \`8px\` (md radius), \`12px\` (lg radius — preferred for most components), \`16px\` (xl radius)
All auto-adapt to light/dark mode. For custom colors in HTML, use CSS variables.

**Dark mode is mandatory** — dark mode is via \`[data-theme="dark"]\` on \`:root\` (though \`prefers-color-scheme\` also works since the \`.canvas-display\` container auto-styles). Every color must work in both modes:
- In SVG: use the pre-built color classes (\`c-blue\`, \`c-teal\`, \`c-amber\`, etc.) for colored nodes — they handle light/dark mode automatically. Never write \`<style>\` blocks for colors.
- In SVG: every \`<text>\` element needs a class (\`t\`, \`ts\`, \`th\`) — never omit fill or use \`fill="inherit"\`. Inside a \`c-{color}\` parent, text classes auto-adjust to the ramp.
- In HTML: always use \`var(--foreground)\` with appropriate opacity for text. Never hardcode colors like color: #333 — invisible in dark mode.
- Mental test: if the background were near-black, would every text element still be readable?

### Links
\`<a href="https://...">\` just works — clicks open in the default browser.
...
Use \`Display\` and output SVG inline in the html parameter. Same technical rules (viewBox, safe area) but the aesthetic is different:
- Fill the canvas — art should feel rich, not sparse
- Bold colors: mix semantic color categories for variety (info blue, success green, warning amber)
- Art is the one place custom \`<style>\` color blocks are fine — freestyle colors, \`[data-theme="dark"]\` for dark mode variants if you want them
...
\`\`\`html
<div style="display: flex; flex-wrap: wrap; gap: 16px; margin-bottom: 8px; font-size: 12px; color: var(--foreground); opacity: 0.65;">
...
**Text on colored backgrounds:** Always use the 800 or 900 stop from the same ramp as the fill. Never use black, gray, or primary foreground on colored fills. **When a box has both a title and a subtitle, they must be two different stops** — title darker (800 in light mode, 100 in dark), subtitle lighter (600 in light, 200 in dark). Same stop for both reads flat; the weight difference alone isn't enough. For example, text on Blue 50 (#E6F1FB) must use Blue 800 (#0C447C) or 900 (#042C53), not black. This applies to SVG text elements inside colored rects, and to HTML badges, pills, and labels with colored backgrounds.
...
For status/semantic meaning in UI (success, warning, danger) use hardcoded oklch values. For categorical coloring in both diagrams and UI, use these ramps.`;
...
Use \`Display\` for diagrams, outputting SVG inline in the html parameter.
...
**For complex topics, use multiple Display calls** — break the explanation into a series of smaller diagrams rather than one dense diagram. Each SVG streams in with its own animation, creating a visual narrative the user can follow step by step.

**Always add prose between diagrams** — never stack multiple Display calls back-to-back without text. Between each diagram, write a short paragraph (in your normal response text, outside the tool call) that explains what the next diagram shows and connects it to the previous one.
...
**Layout**: Prefer single-direction flows (all top-down or all left-right). Keep diagrams simple — max 4-5 nodes per diagram. The container is variable width but typically 400-700px so complex layouts break.
...
The \`.node\` class adds \`cursor: pointer\` for future interactivity.
...
**Prefer interactive over static.** A static cross-section is a good answer; a cross-section you can *operate* is a great one. The decision rule: if the real-world system has a control, give the diagram that control. A water heater has a thermostat — so give the user a slider that shifts the hot/cold boundary, a toggle that fires the burner and animates convection currents. An LLM has input tokens — let the user click one and watch the attention weights re-fan. A cache has a hit rate — let them drag it and watch latency change. Reach for \`Display\` with inline SVG in the html parameter first; only fall back to static SVG when there's genuinely nothing to twiddle.
...
**Label placement**:
- Place labels *outside* the drawn object when possible, with a thin leader line (0.5px dashed, \`color-mix(in oklch, var(--foreground) 42%, transparent)\` stroke) pointing to the relevant part. This keeps the illustration uncluttered.
...
**Static vs interactive**: Static cutaways and cross-sections work best as pure SVG via \`Display\`. If the diagram benefits from controls — a slider that changes a temperature zone, buttons toggling between operating states, live readouts — use \`Display\` with inline SVG for the drawing and HTML controls around it.

**Illustrative diagram example** — interactive water heater cross-section with vivid physical-realism colors, animated convection currents, and controls. Uses \`Display\` with inline SVG: a thermostat slider shifts the hot/cold gradient boundary, a heating toggle animates flames on/off and transitions convection to paused. viewBox is 680x560; tank occupies x=180..440, leaving 140px+ of right margin for labels. Smooth convection paths use \`stroke-dasharray:5 5\` at ~1.6s for a gentle flow feel. A warm-glow overlay on the hot zone pulses subtly when heating is on. Flame shapes use warm gradient fills and clean opacity transitions. Labels sit along the right margin with leader lines.
\`\`\`html
<style>
  ...
  .toggle-track { position:relative;width:32px;height:18px;background:color-mix(in oklch, var(--foreground) 12%, transparent);border-radius:9px;transition:background .2s;display:inline-block; }
  .toggle-track:has(input:checked) { background:oklch(0.55 0.15 250); }
...
<svg width="100%" viewBox="0 0 680 560">
  ...
  <rect x="180" y="55" width="260" height="390" rx="14" fill="none" stroke="var(--foreground)" stroke-width="2.5" opacity=".25"/>
  <rect x="180" y="55" width="260" height="390" rx="14" fill="none" stroke="var(--foreground)" stroke-width="1" opacity=".42"/>
  <!-- Hot pipe out (top right) -->
  <rect x="370" y="14" width="16" height="50" rx="4" fill="url(#pipe-h)"/>
  <path d="M378 14V55" stroke="var(--foreground)" stroke-width="3" stroke-linecap="round" fill="none" opacity=".42"/>
  <!-- Cold pipe in + dip tube (top left) -->
  <rect x="234" y="14" width="16" height="50" rx="4" fill="url(#pipe-c)"/>
  <path d="M242 14V55" stroke="var(--foreground)" stroke-width="3" stroke-linecap="round" fill="none" opacity=".42"/>
  <path d="M242 55V395" stroke="var(--foreground)" stroke-width="2.5" stroke-linecap="round" fill="none" opacity=".3"/>
  ...
  <!-- Burner bar -->
  <rect x="188" y="454" width="244" height="5" rx="2" fill="var(--foreground)" opacity=".42"/>
  <rect x="220" y="462" width="180" height="6" rx="3" fill="var(--foreground)" opacity=".25"/>
  ...
  <!-- Labels (right margin) -->
  <g class="node">
    <line class="leader" x1="386" y1="34" x2="468" y2="70"/><circle cx="386" cy="34" r="2" fill="var(--foreground)" opacity=".42"/>
    <text class="ts" x="474" y="74">Hot water outlet</text></g>
  <g class="node">
    <line class="leader" x1="250" y1="34" x2="468" y2="140"/><circle cx="250" cy="34" r="2" fill="var(--foreground)" opacity=".42"/>
    <text class="ts" x="474" y="144">Cold water inlet</text></g>
  <g class="node">
    <line class="leader" x1="250" y1="260" x2="468" y2="220"/><circle cx="250" cy="260" r="2" fill="var(--foreground)" opacity=".42"/>
    <text class="ts" x="474" y="224">Dip tube</text></g>
  <g class="node">
    <line class="leader" x1="440" y1="250" x2="468" y2="300"/><circle cx="440" cy="250" r="2" fill="var(--foreground)" opacity=".42"/>
    <text class="ts" x="474" y="304">Thermostat</text></g>
  <g class="node">
    <line class="leader" x1="440" y1="380" x2="468" y2="380"/><circle cx="440" cy="380" r="2" fill="var(--foreground)" opacity=".42"/>
    <text class="ts" x="474" y="384">Tank wall</text></g>
  <g class="node">
    <line class="leader" x1="432" y1="454" x2="468" y2="454"/><circle cx="432" cy="454" r="2" fill="var(--foreground)" opacity=".42"/>
    <text class="ts" x="474" y="458">Heating element</text></g>
</svg>
<div style="display:flex;align-items:center;gap:16px;margin:12px 0 0;font-size:13px;color:var(--foreground);opacity:0.65">
...
The \`.canvas-display\` container provides pre-built SVG classes. These are already loaded — do not redefine them in \`<style>\` blocks.
...
**Font size calibration for diagram text labels** - Here's csv table to give you better sense of the Inter font rendering width:
\`\`\`csv
...
**Pre-built classes** (already loaded in the \`.canvas-display\` container):
- \`class="t"\` = sans 14px primary (\`var(--foreground)\` at 0.92 opacity), \`class="ts"\` = sans 12px secondary (\`var(--foreground)\` at 0.65 opacity), \`class="th"\` = sans 14px medium (500, \`var(--foreground)\` at 0.92 opacity)
- \`class="box"\` = neutral rect (\`color-mix(in oklch, var(--foreground) 3%, transparent)\` fill, \`color-mix(in oklch, var(--foreground) 7%, transparent)\` stroke)
- \`class="node"\` = clickable group with hover effect (cursor pointer, slight dim on hover)
- \`class="arr"\` = arrow line (1.5px, open chevron head)
- \`class="leader"\` = dashed leader line (\`color-mix(in oklch, var(--foreground) 42%, transparent)\` stroke, 0.5px, dashed)
...
- Short aliases: \`var(--foreground)\` with opacity 0.92 for primary, \`var(--foreground)\` with opacity 0.65 for secondary, \`var(--foreground)\` with opacity 0.42 for tertiary, \`color-mix(in oklch, var(--foreground) 3%, transparent)\` for bg surfaces, \`color-mix(in oklch, var(--foreground) 7%, transparent)\` for borders
...
### Aesthetic
Flat, clean surfaces. Minimal 0.5px borders. Generous whitespace. No gradients, no shadows (except functional focus rings). Everything should feel native to Stella — like it belongs in the dashboard, not embedded from somewhere else.

### Tokens
- Borders: always \`0.5px solid color-mix(in oklch, var(--foreground) 7%, transparent)\` (or \`12%\` for emphasis)
- Corner radius: \`8px\` for most elements, \`12px\` for cards
- Cards: \`var(--background)\` bg, 0.5px border, 12px radius, padding 1rem 1.25rem
- Form elements (input, select, textarea, button, range slider) are pre-styled — write bare tags. Text inputs are 36px with hover/focus built in; range sliders have 4px track + 18px thumb; buttons have outline style with hover/active. Only add inline styles to override (e.g., different width).
- Buttons: pre-styled with transparent bg, 0.5px border at 12% foreground mix, hover at 3% foreground mix bg, active scale(0.98).
...
### Metric cards
For summary numbers (revenue, count, percentage) — surface card with muted 13px label above, 24px/500 number below. \`background: color-mix(in oklch, var(--foreground) 3%, transparent)\`, no border, \`border-radius: 8px\`, padding 1rem. Use in grids of 2-4 with \`gap: 12px\`. Distinct from raised cards (which have background bg + border).
...
### Mockup presentation
Contained mockups — mobile screens, chat threads, single cards, modals, small UI components — should sit on a background surface (\`color-mix(in oklch, var(--foreground) 3%, transparent)\` container with \`border-radius: 12px\` and padding, or a device frame) so they don't float naked on the panel canvas. Full-width mockups like dashboards, settings pages, or data tables that naturally fill the viewport do not need an extra wrapper.

### 1. Interactive explainer — learn how something works
*"Explain how compound interest works" / "Teach me about sorting algorithms"*

Use \`Display\` for the interactive controls — sliders, buttons, live state displays, charts. Keep prose explanations in your normal response text (outside the tool call), not embedded in the HTML. No card wrapper. Whitespace is the container.

\`\`\`html
<div style="display: flex; align-items: center; gap: 12px; margin: 0 0 1.5rem;">
  <label style="font-size: 13px; color: var(--foreground); opacity: 0.65;">Years</label>
  <input type="range" min="1" max="40" value="20" id="years" style="flex: 1;" />
  <span style="font-size: 13px; font-weight: 500; min-width: 24px;" id="years-out">20</span>
</div>

<div style="display: flex; align-items: baseline; gap: 8px; margin: 0 0 1.5rem;">
  <span style="font-size: 13px; color: var(--foreground); opacity: 0.65;">£1,000 →</span>
  <span style="font-size: 22px; font-weight: 500;" id="result">£3,870</span>
</div>
...
Use \`Display\`. Side-by-side card grid for options. Highlight differences with semantic colors. Interactive elements for filtering or weighting.
...
- When one option is recommended or "most popular", accent its card with \`border: 2px solid oklch(0.55 0.15 250)\` only (2px is deliberate — the only exception to the 0.5px rule, used to accent featured items) — keep the same background and border as the other cards. Add a small badge (e.g. "Most popular") above or inside the card header using \`background: oklch(0.95 0.03 250); color: oklch(0.45 0.15 250); font-size: 12px; padding: 4px 12px; border-radius: 8px\`.

### 3. Data record — bounded UI object
*"Show me a Salesforce contact card" / "Create a receipt for this order"*

Use \`Display\`. Wrap the entire thing in a single raised card. All content is sans-serif since it's pure UI. Use an avatar/initials circle for people (see example below).

\`\`\`html
<div style="background: var(--background); border-radius: 12px; border: 0.5px solid color-mix(in oklch, var(--foreground) 7%, transparent); padding: 1rem 1.25rem;">
  <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 16px;">
    <div style="width: 44px; height: 44px; border-radius: 50%; background: oklch(0.95 0.03 250); display: flex; align-items: center; justify-content: center; font-weight: 500; font-size: 14px; color: oklch(0.45 0.15 250);">MR</div>
    <div>
      <p style="font-weight: 500; font-size: 15px; margin: 0;">Maya Rodriguez</p>
      <p style="font-size: 13px; color: var(--foreground); opacity: 0.65; margin: 0;">VP of Engineering</p>
    </div>
  </div>
  <div style="border-top: 0.5px solid color-mix(in oklch, var(--foreground) 7%, transparent); padding-top: 12px;">
    <table style="width: 100%; font-size: 13px;">
      <tr><td style="color: var(--foreground); opacity: 0.65; padding: 4px 0;">Email</td><td style="text-align: right; padding: 4px 0; color: oklch(0.55 0.15 250);">m.rodriguez@acme.com</td></tr>
      <tr><td style="color: var(--foreground); opacity: 0.65; padding: 4px 0;">Phone</td><td style="text-align: right; padding: 4px 0;">+1 (415) 555-0172</td></tr>
    </table>
  </div>
</div>
\`\`\``;
...
export function getDisplayGuidelines(modules: string[]): string {
  let content = CORE;
