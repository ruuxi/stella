...
export class StellaRuntimeClient {
  ...
  private async spawnDaemon() {
    ...
    const { peer } = attachJsonRpcPeerToStreams({
      ...
      output: child.stdin,
      onError: (error) => {
        console.error("[stella-runtime-client] RPC error:", error);
      },
    });
    this.peer = peer;
      ...
      this.respawnTimer = setTimeout(() => {
        ...
        if (this.started && !this.process) {
          void this.spawnDaemon().catch((error) => {
            console.error("[stella-runtime-client] Failed to respawn daemon:", error);
          });
        }
      }, delayMs);
...
const classifyRuntimeReload = (
  ...
  if (
    normalizedFilename.startsWith("packages/stella-runtime-capabilities/") ||
    normalizedFilename.startsWith("resources/bundled-commands/")
  ) {
    return "capabilities";
  }
  if (
    normalizedFilename.startsWith("packages/stella-runtime-worker/") ||
    normalizedFilename.startsWith("electron/core/runtime/") ||
    normalizedFilename.startsWith("electron/self-mod/") ||
    normalizedFilename.startsWith("electron/storage/") ||
    normalizedFilename === "electron/system/device.js" ||
    normalizedFilename === "electron/services/local-scheduler-service.js"
  ) {
    return "worker";
  }
  if (
    normalizedFilename.startsWith("packages/stella-runtime-daemon/") ||
    normalizedFilename.startsWith("packages/stella-runtime-protocol/") ||
    normalizedFilename.startsWith("packages/stella-runtime-client/")
  ) {
    return "daemon";
  }
  return null;
