...
void main() {
  ...
  // Chromatic aberration color — appears at the split edges
  // Sample slight offsets to detect where the image changes rapidly (edge detection)
  ...
  // Only at edges where chromatic split would be visible — thin color outlines
 ...
 * Controls strength and mix externally via refs.
 */
...
/** Smoothly animate a ref value from its current value to a target over duration ms. */
function tweenRef(
  ref: { current: number },
  to: number,
  duration: number,
  ...
  useEffect(() => {
    ...
    unsubs.push(
      api.onMorphForward((data) => {
        ...
        // Load the initial image and start
        void loadImage(data.screenshotDataUrl).then((img) => {
          ...
          stopLoopRef.current = startRenderLoop(ctx, strengthRef, mixRef, loopStartTimeRef.current);

          // Ramp up strength smoothly
          void tweenRef(strengthRef, STEADY_STRENGTH, RAMP_UP_MS);
        });
      }),
    );
    ...
    unsubs.push(
      api.onMorphReverse((data) => {
        void loadImage(data.screenshotDataUrl).then((img) => {
          ...
          // Load new texture, crossfade, then calm down
