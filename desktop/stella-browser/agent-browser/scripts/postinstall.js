...
/**
 * Postinstall script for stella-browser
 * 
...
const binaryName = `stella-browser-${platformKey}${ext}`;
...
// GitHub release URL
const GITHUB_REPO = 'vercel-labs/stella-browser';
...
async function fixUnixSymlink() {
  ...
  const symlinkPath = join(npmBinDir, 'stella-browser');
...
async function fixWindowsShims() {
  ...
  const cmdShim = join(npmBinDir, 'stella-browser.cmd');
  const ps1Shim = join(npmBinDir, 'stella-browser.ps1');
