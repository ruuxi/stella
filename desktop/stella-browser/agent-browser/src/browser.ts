...
export class BrowserManager {
  ...
  async launch(options: LaunchCommand): Promise<void> {
    ...
    // Cloud browser providers require explicit opt-in via -p flag or STELLA_BROWSER_PROVIDER env var
    ...
    // Kernel: requires explicit opt-in via -p kernel flag or STELLA_BROWSER_PROVIDER=kernel
    if (provider === 'kernel') {
    ...
    if (hasExtensions) {
      ...
      const session = process.env.STELLA_BROWSER_SESSION || 'default';
      ...
      context = await launcher.launchPersistentContext(
        path.join(os.tmpdir(), `stella-browser-ext-${session}`),
        {
          headless: false,
  ...
  async startRecording(outputPath: string, url?: string): Promise<void> {
    ...
    this.recordingTempDir = path.join(
      os.tmpdir(),
      `stella-browser-recording-${session}-${Date.now()}`
    );
