...
let currentSession = process.env.STELLA_BROWSER_SESSION || 'default';
...
export async function startDaemon(options?: {
  ...
  const provider = options?.provider ?? process.env.STELLA_BROWSER_PROVIDER;
  const isIOS = provider === 'ios';
  const isUserBrowser = process.env.STELLA_BROWSER_USER_BROWSER === '1';
  ...
  const streamPort =
    options?.streamPort ??
    (process.env.STELLA_BROWSER_STREAM_PORT
      ? parseInt(process.env.STELLA_BROWSER_STREAM_PORT, 10)
      : 0);
        ...
        try {
          ...
          if (
            ...
            if (isIOS && manager instanceof IOSManager) {
              ...
              const extensions = process.env.STELLA_BROWSER_EXTENSIONS
                ? process.env.STELLA_BROWSER_EXTENSIONS.split(',')
                    .map((p) => p.trim())
                    .filter(Boolean)
                : undefined;

              // Parse args from env (comma or newline separated)
              const argsEnv = process.env.STELLA_BROWSER_ARGS;
              const args = argsEnv
                ? argsEnv
                    .split(/[,\n]/)
                    .map((a) => a.trim())
              ...
              const proxyServer = process.env.STELLA_BROWSER_PROXY;
              const proxyBypass = process.env.STELLA_BROWSER_PROXY_BYPASS;
              ...
              const ignoreHTTPSErrors = process.env.STELLA_BROWSER_IGNORE_HTTPS_ERRORS === '1';
              const allowFileAccess = process.env.STELLA_BROWSER_ALLOW_FILE_ACCESS === '1';
              await manager.launch({
                id: 'auto',
                action: 'launch' as const,
                headless: process.env.STELLA_BROWSER_HEADED !== '1',
                executablePath: process.env.STELLA_BROWSER_EXECUTABLE_PATH,
                extensions: extensions,
                profile: process.env.STELLA_BROWSER_PROFILE,
                storageState: process.env.STELLA_BROWSER_STATE,
                args,
                userAgent: process.env.STELLA_BROWSER_USER_AGENT,
                proxy,
