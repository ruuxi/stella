...
pub fn parse_flags(args: &[String]) -> Flags {
    ...
    let mut flags = Flags {
        ...
        state: env::var("STELLA_BROWSER_STATE").ok(),
        proxy: env::var("STELLA_BROWSER_PROXY").ok(),
        proxy_bypass: env::var("STELLA_BROWSER_PROXY_BYPASS").ok(),
        args: env::var("STELLA_BROWSER_ARGS").ok(),
        user_agent: env::var("STELLA_BROWSER_USER_AGENT").ok(),
        provider: env::var("STELLA_BROWSER_PROVIDER").ok(),
        ignore_https_errors: false,
        allow_file_access: env::var("STELLA_BROWSER_ALLOW_FILE_ACCESS").is_ok(),
        device: env::var("STELLA_BROWSER_IOS_DEVICE").ok(),
