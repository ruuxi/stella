...
const CONFIG_DIR: &str = ".stella-browser";
const CONFIG_FILENAME: &str = "config.json";
const PROJECT_CONFIG_FILENAME: &str = "stella-browser.json";
...
pub fn load_config(args: &[String]) -> Result<Config, String> {
    let explicit = extract_config_path(args)
        .map(|p| ("--config", p))
        .or_else(|| {
            env::var("STELLA_BROWSER_CONFIG")
                .ok()
                .map(|p| ("STELLA_BROWSER_CONFIG", Some(p)))
        });
...
pub fn parse_flags(args: &[String]) -> Flags {
    ...
    let extensions_env = env::var("STELLA_BROWSER_EXTENSIONS")
        .ok()
        .map(|s| {
            s.split(',')
    ...
    let mut flags = Flags {
        json: env_var_is_truthy("STELLA_BROWSER_JSON") || config.json.unwrap_or(false),
        full: env_var_is_truthy("STELLA_BROWSER_FULL") || config.full.unwrap_or(false),
        headed: env_var_is_truthy("STELLA_BROWSER_HEADED") || config.headed.unwrap_or(false),
        debug: env_var_is_truthy("STELLA_BROWSER_DEBUG") || config.debug.unwrap_or(false),
        session: env::var("STELLA_BROWSER_SESSION")
            .ok()
            .or(config.session)
            .unwrap_or_else(|| "default".to_string()),
        headers: config.headers,
        executable_path: env::var("STELLA_BROWSER_EXECUTABLE_PATH")
            .ok()
            .or(config.executable_path),
        cdp: config.cdp,
        extensions,
        profile: env::var("STELLA_BROWSER_PROFILE").ok().or(config.profile),
        state: env::var("STELLA_BROWSER_STATE").ok().or(config.state),
        proxy: env::var("STELLA_BROWSER_PROXY").ok().or(config.proxy),
        proxy_bypass: env::var("STELLA_BROWSER_PROXY_BYPASS")
            .ok()
            .or(config.proxy_bypass),
        args: env::var("STELLA_BROWSER_ARGS").ok().or(config.args),
        user_agent: env::var("STELLA_BROWSER_USER_AGENT")
            .ok()
            .or(config.user_agent),
        provider: env::var("STELLA_BROWSER_PROVIDER").ok().or(config.provider),
        ignore_https_errors: env_var_is_truthy("STELLA_BROWSER_IGNORE_HTTPS_ERRORS")
            || config.ignore_https_errors.unwrap_or(false),
        allow_file_access: env_var_is_truthy("STELLA_BROWSER_ALLOW_FILE_ACCESS")
            || config.allow_file_access.unwrap_or(false),
        device: env::var("STELLA_BROWSER_IOS_DEVICE").ok().or(config.device),
        auto_connect: env_var_is_truthy("STELLA_BROWSER_AUTO_CONNECT")
            || config.auto_connect.unwrap_or(false),
        session_name: env::var("STELLA_BROWSER_SESSION_NAME")
            .ok()
            .or(config.session_name),
        annotate: env_var_is_truthy("STELLA_BROWSER_ANNOTATE") || config.annotate.unwrap_or(false),
        color_scheme: env::var("STELLA_BROWSER_COLOR_SCHEME")
            .ok()
            .or(config.color_scheme),
        download_path: env::var("STELLA_BROWSER_DOWNLOAD_PATH")
            .ok()
            .or(config.download_path),
        content_boundaries: env_var_is_truthy("STELLA_BROWSER_CONTENT_BOUNDARIES")
            || config.content_boundaries.unwrap_or(false),
        max_output: env::var("STELLA_BROWSER_MAX_OUTPUT")
            .ok()
            .and_then(|s| s.parse().ok())
            .or(config.max_output),
        allowed_domains: env::var("STELLA_BROWSER_ALLOWED_DOMAINS")
            .ok()
            .map(|s| {
                s.split(',')
        ...
        action_policy: env::var("STELLA_BROWSER_ACTION_POLICY")
            .ok()
            .or(config.action_policy),
        confirm_actions: env::var("STELLA_BROWSER_CONFIRM_ACTIONS")
            .ok()
            .or(config.confirm_actions),
        confirm_interactive: env_var_is_truthy("STELLA_BROWSER_CONFIRM_INTERACTIVE")
            || config.confirm_interactive.unwrap_or(false),
        engine: env::var("STELLA_BROWSER_ENGINE").ok().or(config.engine),
        screenshot_dir: env::var("STELLA_BROWSER_SCREENSHOT_DIR")
            .ok()
            .or(config.screenshot_dir),
        screenshot_quality: env::var("STELLA_BROWSER_SCREENSHOT_QUALITY")
            .ok()
            .and_then(|s| s.parse().ok())
            .or(config.screenshot_quality),
        screenshot_format: env::var("STELLA_BROWSER_SCREENSHOT_FORMAT")
            .ok()
            .or(config.screenshot_format)
            .filter(|s| s == "png" || s == "jpeg"),
        idle_timeout: parse_idle_timeout_value(
            env::var("STELLA_BROWSER_IDLE_TIMEOUT_MS").ok(),
...
mod tests {
    ...
    fn test_load_config_missing_file_returns_none() {
        let result = read_config_file(&PathBuf::from("/nonexistent/stella-browser.json"));
