...
pub async fn save_state(
    ...
    if let Ok(key) = std::env::var("STELLA_BROWSER_ENCRYPTION_KEY") {
...
pub async fn load_state(client: &CdpClient, session_id: &str, path: &str) -> Result<(), String> {
    let json_str = if path.ends_with(".enc") {
        let key = std::env::var("STELLA_BROWSER_ENCRYPTION_KEY").map_err(|_| {
            "Encrypted state file requires STELLA_BROWSER_ENCRYPTION_KEY".to_string()
        })?;
        let data =
...
mod tests {
    ...
    fn test_state_show_nonexistent_file() {
        let result = state_show("/tmp/nonexistent-stella-browser-state-file.json");
        assert!(result.is_err());
    }

    #[test]
    fn test_state_clear_nonexistent_file() {
        let result = state_clear(Some("/tmp/nonexistent-stella-browser-state-file.json"));
        assert!(result.is_err());
    }

    #[test]
    fn test_state_rename_nonexistent() {
        let result = state_rename("/tmp/nonexistent-stella-browser-state-file.json", "new-name");
        assert!(result.is_err());
