...
import { BrowserManager, getNonEmptyContextPages } from '../../stella-browser/src/browser.js';
...
describe('BrowserManager', () => {
  ...
  describe('removed auth helpers', () => {
    it('does not expose legacy scoped-header helpers anymore', () => {
      expect('setScopedHeaders' in browser).toBe(false);
      expect('clearScopedHeaders' in browser).toBe(false);
    });
  });
  ...
  describe('CDP session', () => {
    ...
    it('should filter out pages with empty URLs during CDP connection', async () => {
      ...
      const pages = getNonEmptyContextPages(mockBrowser.contexts() as Array<{
