...
const ENABLE_BUNDLED_DEV = true;
const REACT_REFRESH_PREAMBLE_ENTRY_NAME = "react-refresh-preamble";
const REACT_REFRESH_PREAMBLE_ENTRY_FILE = path.resolve(
  __dirname,
  "src",
  "platform",
  "dev",
  "vite-react-refresh-preamble.ts",
);
const REACT_REFRESH_PREAMBLE_SCRIPT_TAG = `<script type="module" crossorigin src="/assets/${REACT_REFRESH_PREAMBLE_ENTRY_NAME}.js"></script>`;
const DEV_URL_FILE = path.resolve(__dirname, ".vite-dev-url");
const SELF_MOD_HMR_STATE_FILE = path.resolve(
  __dirname,
  ".stella-hmr-state.json",
);
...
const toViteFsPath = (filePath: string) =>
  `/@fs/${filePath.replace(/\\/g, "/")}`;
  ...
  return {
    ...
    configureServer(server) {
      try {
        ...
        console.warn(
          "[workspace-panel-server] Failed to create runtime panels directory:",
          error,
        );
      }

      server.middlewares.use(async (req, res, next) => {
        if (!req.url || !req.url.startsWith("/workspace/panels/"))
          return next();
...
const injectReactRefreshPreambleScript = (html: string) => {
  if (html.includes(REACT_REFRESH_PREAMBLE_SCRIPT_TAG)) {
    return html;
  }

  return html.replace(
    /<script type="module" crossorigin src="\/assets\/(?:main|overlay)\.js"><\/script>/,
    `${REACT_REFRESH_PREAMBLE_SCRIPT_TAG}\n    $&`,
  );
};

type FullBundleMemoryFile = {
  source: string | Uint8Array;
};

type FullBundleClientEnvironment = {
  memoryFiles?: Map<string, FullBundleMemoryFile>;
  triggerBundleRegenerationIfStale?: () => Promise<boolean>;
};

function bundledDevReactPreamble(): Plugin {
  return {
    name: "bundled-dev-react-preamble",
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        if (!server.config.experimental.bundledDev || !req.url) {
          return next();
        }
        ...
        const requestedHtml =
          urlPath === "/"
            ? "index.html"
            : urlPath === "/index.html" || urlPath === "/overlay.html"
              ? urlPath.slice(1)
              : null;
        ...
        const fullBundleClient = server.environments
          .client as FullBundleClientEnvironment;
        if (
          !fullBundleClient.memoryFiles ||
          typeof fullBundleClient.triggerBundleRegenerationIfStale !==
            "function"
        ) {
          return next();
        }

        await fullBundleClient.triggerBundleRegenerationIfStale();
        const file = fullBundleClient.memoryFiles.get(requestedHtml);
        ...
        res.end(injectReactRefreshPreambleScript(source));
      });
    },
  };
}

function bundledDevJsxDevRuntimeFix(): Plugin {
  return {
    name: "bundled-dev-jsxdev-runtime-fix",
    apply: "serve",
    enforce: "post",
    renderChunk(code) {
      if (!ENABLE_BUNDLED_DEV || !code.includes("(void 0)(")) {
        return null;
      }

      const jsxDevRuntimeMatch = code.match(
        /var ([A-Za-z_$][\w$]*) = require_jsx_dev_runtime\(\);/,
      );
      const jsxDevRuntimeRef = jsxDevRuntimeMatch?.[1];
      ...
      const patched = code.replace(
        /\/\* @__PURE__ \*\/ \(void 0\)\(/g,
        `/* @__PURE__ */ (0, ${jsxDevRuntimeRef}.jsxDEV)(`,
      );
  ...
  return {
    ...
    configureServer(server) {
      ...
      const flushQueuedUpdates = async () => {
        ...
        for (const mod of queuedModules) {
          try {
            ...
            console.error(
              "[self-mod-hmr] Failed to reload queued module:",
              error,
            );
            reloadFailed = true;
            break;
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        if (
          req.method === "POST" &&
          urlPath === `${SELF_MOD_HMR_ENDPOINT_BASE}/pause`
        ) {
          paused = true;
          writePersistedSelfModHmrState({ paused, requiresFullReload });
        ...
        if (
          req.method === "POST" &&
          urlPath === `${SELF_MOD_HMR_ENDPOINT_BASE}/resume`
        ) {
          paused = false;
...
export default defineConfig(({ command }) => ({
  plugins: [
    ...
    bundledDevReactPreamble(),
    bundledDevJsxDevRuntimeFix(),
    selfModHmrControl(),
  ],
  base: "./",
  define: {
    __STELLA_BUNDLED_DEV__: JSON.stringify(ENABLE_BUNDLED_DEV),
  },
  experimental: {
    bundledDev: ENABLE_BUNDLED_DEV,
  },
  build: {
    outDir: "dist",
    target: "chrome134",
    rollupOptions: {
      input: {
        ...
        ...(command === "serve"
          ? {
              [REACT_REFRESH_PREAMBLE_ENTRY_NAME]:
                REACT_REFRESH_PREAMBLE_ENTRY_FILE,
            }
          : {}),
      },
      output: {
